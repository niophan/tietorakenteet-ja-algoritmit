...
void initialize_p5(int &end_time, int &queue_limit,
...

{
   cout << "This program simulates an airport with only one runway." << endl
        << "To each arrive plane, it will have a randome fuel level." << endl
...

{
   ...
   for (int current_time = 0; current_time < end_time; current_time++) { //  loop over time intervals
      ...
      for (int i = 0; i < number_arrivals; i++) {
         int fuel = variable.random_integer(1, 10); // Giving a random integer number for fuel value
         Plane current_plane(flight_number++, current_time, arriving, fuel);
         if (small_airport.can_land_with_fuel(current_plane) != success)
      ...
      Plane moving_plane;
      switch (small_airport.emergency_activity(current_time, moving_plane)) {
      //  Let at most one Plane onto the Runway at current_time.
         ...
         case crash:
            ...
            break;
         case idle:
            run_idle_p5(current_time);
      }
   }
   small_airport.short_fuel_detail();
