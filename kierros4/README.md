1. Which of these is easiest to write?
**Version A** on the easiest to write since there is no any loop or extra implementation with top,pop,empty.
2. Which will run most quickly if the stack is nearly full? Which will run most quickly if the stack is nearly empty? 
...
3. Which would be the best method if the implementation might be changed?
The question is not very clear in what kind of implementation, so **no idea**.
4. In which could we pass the parameter source as a constant reference?
